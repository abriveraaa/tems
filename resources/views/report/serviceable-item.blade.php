...
<body>
    ...
    <div style="margin-top:2rem; text-align: center; text-transform: uppercase;font-family: Calibri;">
        <h3>LIST OF ALL SERVICEABLE ITEM</h3>
    </div>
        ...
        @foreach($serviceitem as $service)
            <tbody>
                <tr style="font-size:11pt;">
                    <td style="text-align:left; padding: 2px 5px;">{{ $service->barcode }}</td>
                    @foreach($service->toolcategory as $category)
                    <td style="text-align:left; padding: 2px 5px;">{{ $category->description }}</td>
                    @endforeach
                    @foreach($service->toolname as $name)
